...
#define MSECS_CALC_RAIN 10000   //every 5 minuteS
...
//    while (!Serial) {
      ...
      // if you wait for the serial port to begin, you'll never get these pins setup. not sure why
...
//    }
//
//  Serial.println("Debug 1: GPRSSerial and Serial started");
  #ifdef AVR
  Wire.begin();
  #else
...
// this is set the last time you upload, so if you leave this in and then restart the sketch it goes
// back to that time.  causes craziness.
//getting some bizarre clock drift, so added this back in 13aug14: again, I think i'm running out of ram
  if (! rtc.isrunning()) {
...
//  Serial.println("Debug 4: Shield powered up");
  ...
  dataString = "PRA: ";
  // if dataString is longer than PRA:12345678 then it won't work, and if you have a debug string 5 it won't work.....
  //  but then again it is late, but that seemed to fix it. fwb-10aug14
  recordRainandTime();
  freeramresults = freeRam();
  Serial.println(freeramresults);
//  Serial.println("Debug 5: recordRainandTime rain once");
//  powerUpOrDown(); //turn shield back off
}
...
//    dataString += (" hundredths of an inch");
    ...
    File dataFile = SD.open("vienna.txt", FILE_WRITE);
    ...
    // if the file isn't open, pop up an error:
      ...
      Serial.println("error opening vienna.txt");
    } 
...
void SendTextEmail()
{
  ...
  delay(100);
//  GPRSSerial.println("AT + CMGS = \"+17065139486\"");//send sms message, be careful need to add a country code before the cellphone number
//500 for t-mobile?  GPRSSerial.println("AT + CMGS = \"111\"");//send sms message, be careful need to add a country code before the cellphone number
  GPRSSerial.println("AT + CMGS = \"500\"");//send sms message, be careful need to add a country code before the cellphone number
  delay(100);
  GPRSSerial.print("rainfall@watergeorgia.com Rainfall:");//the content of the message
  delay(100);
  GPRSSerial.print("\r");
  delay(100);
  GPRSSerial.print("\r");
  delay(100);
  GPRSSerial.print(dataString);//the content of the message
  delay(100);
  GPRSSerial.print("\r");
  delay(100);
//  GPRSSerial.println("Test Message");//the content of the message
//  delay(100);
...
void SendTextMessage()
{
  GPRSSerial.print("AT+CMGF=1\r");    //Because we want to send the SMS in text mode
  delay(100);
  GPRSSerial.println("AT + CMGS = \"+17064145535\"");//send sms message, be careful need to add a country code before the cellphone number
  delay(100);
  GPRSSerial.println(dataString);//the content of the message
  delay(100);
  GPRSSerial.println((char)26);//the ASCII code of the ctrl+z is 26
  delay(100);
  GPRSSerial.println();
